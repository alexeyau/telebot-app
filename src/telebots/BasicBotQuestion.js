...
export class BasicBotQuestion extends BasicBot {
  ...
  async _sendResponse(update) {
    ...
    if(update.message?.text === '/start') {
      this.sendTelegramMessageAsync(update.message?.from.id, 'Привет! Ответьте на первый вопрос:\n' + questions[getStorageItem("numberOfQuestion")].question);
      this._onSend(update);
      return;
    };
    ...
    if(update.message?.text === questions[getStorageItem("numberOfQuestion")].answer) {
      ...
      if(Number(getStorageItem("numberOfQuestion")) == questions.length) {
        this.sendTelegramMessageAsync(update.message?.from.id, 'Правильно, вы ответили на все вопросы');
        this._onSend(update);
        setStorageItem("numberOfQuestion", 0)
        return;
      }
      this.sendTelegramMessageAsync(update.message?.from.id, 'Правильно, вот следующий вопрос:\n' + questions[getStorageItem("numberOfQuestion")].question);
      this._onSend(update);
      return;
    } else {
